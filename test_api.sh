...
AUTH_RESPONSE=$(curl -s -X POST $BASE_URL/api/auth -H "Content-Type: application/json" -d '{"username": "testuser", "password": "password"}')
...
curl -X GET $BASE_URL/api/info -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" 
echo ""

# 2. Получить список товаров пользователя (merch)
echo "3. Получение списка товаров пользователя..."
curl -X GET $BASE_URL/me/merch -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" | jq
echo ""

# 3. Перевести монеты пользователю
echo "4. Перевод монет..."
TRANSFER_RESPONSE=$(curl -s -X POST $BASE_URL/api/sendCoin -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" -d '{"toUser": "anotheruser", "amount": 50}')
